...
const generateAccessAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(500, "Failed to generate refresh andaccess token");
  }
};
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  // validation of email and password - not empty, email format, password strength etc
  // check if user exists in database using email
  // if user does not exist, return error response
  // if user exists, compare the password with the hashed password in database using bcrypt
  // if password is incorrect, return error response
  // if password is correct, generate access token and refresh token using jwt
  ...
  if (!user) {
    throw new ApiError(404, "user is required to register");
  }
  ...
  const options = {
    ...
    sameSite: "lax", // ← Very important for development
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const options = {
      httpOnly: true,
      secure: false, // false because we are on localhost (http)
      sameSite: "lax", // "lax" works best for development
      maxAge: 7 * 24 * 60 * 60 * 1000, // 7 days
    };
